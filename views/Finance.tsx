...
const FinanceView: React.FC<FinanceViewProps> = ({ cityId }) => {
  ...
  const cityTeams = mockTeams.filter(t => t.city === 'Tatuí'); // Simplificado para o exemplo

  const getTeamStats = (teamId: string) => {
    const members = mockMembers.filter(m => m.teamId === teamId);
    const payments = mockPayments.filter(p => p.teamId === teamId && p.referenceMonth === selectedMonth);
    const paidCount = payments.filter(p => p.status === 'Pago').length;
    return {
      total: members.length,
      paid: paidCount,
      percent: members.length > 0 ? (paidCount / members.length) * 100 : 0
    };
  };
  ...
  const renderTeamDetail = () => {
    ...
    const members = mockMembers.filter(m => m.teamId === selectedTeamId);
      ...
      <div className="space-y-6 animate-in slide-in-from-right duration-300">
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-gray-500 text-sm font-medium">Lançamentos referentes ao mês de <strong>{selectedMonth}</strong></p>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {members.map(member => {
                const payment = mockPayments.find(p => p.memberId === member.id && p.referenceMonth === selectedMonth);
                return (
