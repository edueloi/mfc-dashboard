...
import { mockUsers, mockMembers, mockCities } from '../mockData';
              ...
              {filteredUsers.map(user => (
                <tr key={user.id} className="hover:bg-blue-50/20 transition-all group">
                  ...
                  <td className="px-10 py-5 text-sm font-bold text-gray-500">
                    {mockCities.find(c => c.id === user.cityId)?.name || 'Tatuí'} - SP
                  </td>
          ...
          <div className="bg-white w-full max-w-2xl max-h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col animate-in slide-in-from-bottom-5 duration-500 overflow-hidden border border-gray-100">
            ...
            <div className="px-8 py-6 border-b border-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-gray-900 tracking-tight leading-none mb-1">Novo Acesso</h3>
                  <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Criar credenciais de acesso ao sistema</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 hover:bg-gray-50 rounded-xl transition-all text-gray-300 hover:text-gray-500 active:scale-90"><X className="w-6 h-6" /></button>
            </div>
