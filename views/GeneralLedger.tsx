...
import { FinancialEntity, FinancialLaunch } from '../types';
...
const GeneralLedger: React.FC = () => {
  ...
  const [launches, setLaunches] = useState<FinancialLaunch[]>([]);

  useEffect(() => {
    const fetchEntities = async () => {
      try {
        const data = await api.getFinancialEntities();
        setEntities(data);
      } catch (err) {
        console.error('Failed to fetch financial entities', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (selectedEntityId) {
      const fetchLaunches = async () => {
        try {
          const data = await api.getFinancialLaunches(selectedEntityId);
          setLaunches(data);
        } catch (err) {
          console.error('Failed to fetch financial launches', err);
        }
      };
      fetchLaunches();
    }
  ...
  const handleCreateEntity = async () => {
    ...
    try {
      const entity = await api.createFinancialEntity({
        name: newEntity.name,
        year: newEntity.year,
        createdBy: 'Admin',
      ...
      console.error('Failed to create financial entity', err);
      alert('Erro ao criar livro caixa');
    }
  };
  ...
  const handleLaunchSubmit = async () => {
    ...
    try {
      const promises = launchForm.months.map(monthIdx => 
        api.createFinancialLaunch({
          ...
          month: monthIdx,
          year: selectedEntity?.year || new Date().getFullYear()
        })
      );
      await Promise.all(promises);
      
      // Refresh launches
      const updatedLaunches = await api.getFinancialLaunches(selectedEntityId);
      setLaunches(updatedLaunches);

      notify(`Lançamento de ${launchForm.type} realizado com sucesso!`);
      setShowLaunchModal(false);
      setLaunchForm({ type: 'Receita', category: '', amount: '', description: '', months: [] });
    } catch (err) {
      console.error('Failed to create financial launches', err);
            ...
            <tbody className="text-[13px]">
                ...
                <tr key={acc.id} className="hover:bg-blue-50/10">
                  ...
                  {monthNames.map((_, i) => {
                    const monthTotal = launches
                      .filter(l => l.category === acc.label && l.month === i && l.type === 'Receita')
                      .reduce((sum, l) => sum + l.amount, 0);
                    return (
                      <td key={i} className="px-4 py-4 text-right border-r border-b border-gray-50 text-green-600 font-bold tabular-nums">
                        {monthTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                    );
                  })}
                  <td className="px-6 py-4 text-right bg-blue-50/10 border-b border-gray-50 font-black text-blue-700 tabular-nums">
                    {launches
                      .filter(l => l.category === acc.label && l.type === 'Receita')
                      .reduce((sum, l) => sum + l.amount, 0)
              ...
              {categories.Despesa.map((acc, idx) => (
                <tr key={acc.id} className="hover:bg-red-50/10">
                  ...
                  {monthNames.map((_, i) => {
                    const monthTotal = launches
                      .filter(l => l.category === acc.label && l.month === i && l.type === 'Despesa')
                      .reduce((sum, l) => sum + l.amount, 0);
                    return (
                      <td key={i} className="px-4 py-4 text-right border-r border-b border-gray-50 text-red-500 font-medium tabular-nums">
                        {monthTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                    );
                  })}
                  <td className="px-6 py-4 text-right bg-gray-50/50 border-b border-gray-50 font-black text-red-700 tabular-nums">
                    {launches
                      .filter(l => l.category === acc.label && l.type === 'Despesa')
                      .reduce((sum, l) => sum + l.amount, 0)
                      .toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              ...
              <tr className="bg-blue-600 text-white">
                ...
                <td colSpan={13} className="px-10 py-6 text-right font-black text-2xl tracking-tight pr-16 italic">
                  R$ {(
                    (selectedEntity?.initialBalance || 0) + 
                    launches.filter(l => l.type === 'Receita').reduce((sum, l) => sum + l.amount, 0) - 
                    launches.filter(l => l.type === 'Despesa').reduce((sum, l) => sum + l.amount, 0)
                  ).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </td>
              </tr>
            </tbody>
  ...
  return (
    <div className="h-full flex flex-col space-y-6 pb-10">
      ...
      <Modal
        ...
        title="Novo Livro"
        description="Controle de balancete por exercício."
        icon={<Layers className="w-6 h-6" />}
      >
        <div className="space-y-6">
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Nome do Livro</label>
            <input 
              type="text" 
              placeholder="Ex: Livro Caixa 2024" 
              className="w-full bg-gray-50 border border-gray-100 rounded-xl px-5 py-3.5 font-black text-gray-900 outline-none" 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Exercício</label>
              <input 
                type="number" 
                className="w-full bg-gray-50 border border-gray-100 rounded-xl px-5 py-3.5 font-black text-gray-900 outline-none" 
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="sm:col-span-2">
              ...
              <Dropdown
                ...
                onChange={(val) => setLaunchForm({...launchForm, category: val})}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <Button 
              ...
              className={`flex-[2] ${launchForm.type === 'Receita' ? 'bg-green-600 shadow-green-100 hover:bg-green-700' : 'bg-red-500 shadow-red-100 hover:bg-red-600'}`}
              icon={<Save className="w-4 h-4" />}
            >
              Processar Lançamentos
            </Button>
            <Button variant="ghost" onClick={() => setShowLaunchModal(false)} className="flex-1">
              CANCELAR
            </Button>
          </div>
        </div>
