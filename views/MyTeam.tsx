  ...
  const financeStats = useMemo(() => {
    ...
    membersState.forEach(m => {
       for(let mIdx = 1; mIdx <= viewMonth; mIdx++) {
          if(!localPayments.some(p => p.memberId === m.id && p.referenceMonth === `${mIdx}/${viewYear}`)) pendingAmount += 50.00;
       }
    });
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      {activeTab === 'membros' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {membersState.map(m => (
            <div key={m.id} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl transition-all cursor-pointer group" onClick={() => onOpenMember(m.id)}>
      ...
      {/* MODAL MULTI-MESES - ZÉ, AQUI É ONDE VOCÊ MARCA TUDO DE UMA VEZ */}
      {showPayModal && selectedForPayment && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-slate-900/70 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-500">
                <div className="p-10 border-b border-gray-50 bg-white text-center flex flex-col items-center gap-4">
                    <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-[2rem] flex items-center justify-center shadow-inner">
                        <DollarSign className="w-10 h-10" />
                    </div>
                    <div>
                        <h3 className="text-2xl font-black text-gray-900 tracking-tight leading-none">Confirmar Recebimento</h3>
                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] mt-2">MARQUE OS MESES ABAIXO PARA LANÇAR EM LOTE</p>
                    </div>
                </div>
                ...
                <div className="p-10 space-y-10 overflow-y-auto max-h-[60vh] no-scrollbar">
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 flex items-center justify-between">
                        <div>
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Membro/Casal</p>
                          <p className="text-xl font-black text-gray-800">{selectedForPayment.displayName}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Total a Lançar</p>
                          <p className="text-2xl font-black text-emerald-600">R$ {(paymentForm.months.length * paymentForm.amountPerMonth).toFixed(2)}</p>
                        </div>
                    </div>

                    <div className="space-y-4">
                       <div className="flex items-center justify-between px-2">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Selecione os Meses ({paymentForm.year})</label>
                          <select className="bg-transparent border-none text-[10px] font-black text-blue-600 outline-none" value={paymentForm.year} onChange={(e) => setPaymentForm({...paymentForm, year: parseInt(e.target.value)})}>
                             {[2024, 2025, 2026, 2027].map(y => <option key={y} value={y}>{y}</option>)}
                          </select>
                       </div>
                          ...
                          {monthNames.map((m, i) => {
                            ...
                            return (
                              <button 
                                ...
                                className={`relative group p-4 rounded-2xl border transition-all flex flex-col items-center gap-1 ${isPaid ? 'bg-emerald-50 border-emerald-100 cursor-not-allowed grayscale' : isSelected ? 'bg-blue-600 border-blue-600 text-white shadow-xl scale-[1.05]' : 'bg-gray-50 border-gray-100 text-gray-500 hover:bg-white hover:border-blue-200'}`}
                              >
                                 <span className="text-[9px] font-black uppercase tracking-widest">{shortMonths[i]}</span>
                                 {isPaid ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> : isSelected ? <Check className="w-3.5 h-3.5" /> : <div className="w-3.5 h-3.5 rounded-full border-2 border-gray-200 group-hover:border-blue-300"></div>}
                                 {isPaid && <span className="absolute -top-2 -right-2 bg-emerald-500 text-white text-[7px] font-black px-1.5 py-0.5 rounded-full shadow-lg">PAGO</span>}
                              </button>
                            );
                          })}
                       </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea 
                            ...
                            className="w-full bg-gray-50 border border-gray-100 rounded-xl px-5 py-4 text-sm font-bold text-gray-600 outline-none focus:ring-4 focus:ring-blue-50 resize-none"
                ...
                <div className="p-10 bg-gray-50 border-t border-gray-100 flex flex-col gap-4">
                    <button 
                        ...
                        className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50 disabled:grayscale"
                    >
                        <Save className="w-5 h-5" /> Confirmar Lançamento ({paymentForm.months.length} Meses)
                    </button>
                    <button 
